...
// Example script with tiny edits
namespace Pathfinding.Examples {
	...
	public class RVOExampleAgent : MonoBehaviour {
		...
		public void Update () {
			...
			if (!controller.locked && Time.time >= nextRepath && canSearchAgain) {
			...
			if (!controller.locked && vectorPath != null && vectorPath.Count != 0) {
				...
				var netRemainingDistance                                                   = (transform.position - vectorPath[^1]).magnitude;
				
				if(controller.lockWhenNotMoving && netRemainingDistance <2) controller.locked = controller.velocity.magnitude / maxSpeed < 0.25f;
				if(controller.locked) controller.SetTarget(pos, maxSpeed, maxSpeed);
				else controller.SetTarget(rvoTarget, desiredSpeed, maxSpeed);
			} else {
				// Stand still
